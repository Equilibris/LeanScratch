...
noncomputable def denote : Expr Γ ret → CFunc (context Γ) ret.denote
  | .fix v => fix'.comp v.denote
  | .app fn arg => mp'.comp (CFunc.corecP fn.denote arg.denote)
  | .abs _ body => curry $ CFunc.swap body.denote
  | .bvar idx => proj idx
  | .eif cond t f => eif.denote.comp (corecP cond.denote (corecP t.denote f.denote))
...
example : (Expr.abs .nat (.bvar (Γ := [.nat]) .fz)).denote.f = λ _ ↦ { f := id, continous := by infer_instance } := rfl
example : (Expr.abs .nat (.succ $ .bvar (Γ := [.nat]) .fz)).denote.f = λ _ ↦ { f := succ.denote', continous := by infer_instance } := rfl
